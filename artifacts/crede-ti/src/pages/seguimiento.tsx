        ...
        {data && sm && (
          <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ background: sm.bg, borderRadius: 18, border: `1.5px solid ${sm.color}33`, padding: 18, textAlign: "center" }}>
              ...
              {data.requestedAmount != null && <div style={{ fontSize: 13, color: "var(--text-secondary)", marginTop: 8 }}>Monto solicitado: <strong>{fmtMoney(data.requestedAmount)}</strong></div>}
              <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4 }}>Solicitud del {fmtDate(data.createdAt)}</div>
            </div>
