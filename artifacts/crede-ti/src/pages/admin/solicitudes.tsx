...
function CreditCard({ credit, onClick }: { credit: PendingCredit; onClick: () => void }) {
  ...
  return (
    <div
      ...
      onClick={onClick}
      style={needsInfo ? { borderLeft: "4px solid var(--warning)" } : {}}
    >
    ...
    <div className="flex flex-col gap-4 pb-2">
      <div className="flex items-center justify-between gap-3">
        ...
        <button onClick={() => setEditMode(e => !e)}
          ...
          style={{ background: editMode ? "var(--surface-3)" : "var(--surface-3)", color: editMode ? "var(--danger)" : "#215DFF" }}>
      ...
      {editMode ? (
        ...
        <div className="bg-gray-50 rounded-2xl p-4 flex flex-col gap-0">
          ...
          {[
            ...
            { label: "Intereses",      val: fmt(credit.totalToRepay - credit.amount), color: "var(--warning)" },
      ...
      <button onClick={() => { if (!notes.trim()) return; mut.mutate({ action: "needs_info", n: notes }); }}
        ...
        style={{ background: notes.trim() ? "linear-gradient(135deg,var(--warning),#fbbf24)" : "var(--border)", color: notes.trim() ? "#451a03" : "var(--text-muted)", border: "none" }}>
    ...
    <div className="flex flex-col gap-4 pb-2">
      ...
      <div className="rounded-2xl p-3.5 flex flex-col gap-3" style={{ background: "#fff", border: "1.5px solid var(--border)" }}>
        ...
        <div className="flex flex-col gap-2" style={{ borderTop: "1px solid var(--surface-2)", paddingTop: 10 }}>
          ...
          {notify && !dEmail && (
            <div className="text-[11px]" style={{ color: "var(--warning)" }}>Sin correo registrado {"\u2014"} el aviso no se enviara. Contactalo al {dPhone}.</div>
          )}
            ...
            {form.references.map((r, idx) => (
              <div key={idx} className="rounded-xl p-2 flex flex-col gap-1.5" style={{ background: "#fff", border: "1px solid var(--border)" }}>
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setForm(f => ({ ...f, references: f.references.filter((_, i) => i !== idx) }))} className="text-[11px] font-semibold" style={{ color: "var(--danger)" }}>Quitar</button>
                </div>
      ...
      {editing && (
        <div className="flex flex-col gap-2 sticky bottom-0 pb-1" style={{ background: "linear-gradient(to top, #fff 70%, transparent)" }}>
          {detailsM.isError && <div className="text-xs text-center" style={{ color: "var(--danger)" }}>{(detailsM.error as Error).message}</div>}
          <div className="flex gap-2">
