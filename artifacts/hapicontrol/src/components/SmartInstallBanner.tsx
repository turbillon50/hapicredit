...
export default function SmartInstallBanner() {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("hapi_install_dismissed")) return;
  ...
  function dismiss() {
    localStorage.setItem("hapi_install_dismissed", "1");
  ...
  async function installAndroid() {
    ...
    if (outcome === "accepted") localStorage.setItem("hapi_install_dismissed", "1");
    setShowAndroid(false);
  ...
  if (showAndroid && deferredPrompt) {
    return (
      <div style={{
        ...
        background: "#142246", borderRadius: 20,
        padding: "14px 16px",
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 800, fontSize: 14, color: "#fff", lineHeight: 1.2 }}>Instalar HapiCredit</div>
      ...
      >
        <div style={{
          ...
          <div style={{ fontSize: 13, color: "#64748b", marginBottom: 18, lineHeight: 1.5 }}>
            Agrega HapiCredit a tu pantalla de inicio para usarla como app nativa, sin abrir el navegador.
          </div>
          {[
