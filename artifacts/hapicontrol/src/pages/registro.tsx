...
export default function Registro() {
  ...
  async function handleValidateCode() {
    ...
    if (!trimmed) { setCodeError("Ingresa el código de invitación"); return; }
    setCodeLoading(true);
    try {
      ...
      // Role check: code role must match selected role (except admin can use any admin code)
      if (selectedRole && data.role !== selectedRole) {
        setCodeError(`Este código es para ${ROLE_LABEL[data.role]}, no para ${ROLE_LABEL[selectedRole]}`);
        return;
      }
      setCode(trimmed);
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      setStep("done");
      setTimeout(() => navigate(roleHome(data.user.role)), 2200);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "var(--navy-800)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px" }}>
      ...
      <div style={{ marginBottom: 28, textAlign: "center" }}>
        ...
        <div style={{ color: "white", fontWeight: 700, fontSize: 20, marginTop: 6 }}>HapiCredit</div>
        <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 12 }}>Tu crédito, Tu impulso</div>
      </div>

      <div style={{ background: "white", borderRadius: 20, padding: "28px 24px", width: "100%", maxWidth: 400 }}>

        {/* ── STEP: ROLE SELECTOR ─────────────────────────────────────── */}
        {step === "role" && (
          <>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "var(--navy-800)", marginBottom: 6 }}>Crear cuenta</h2>
            <p style={{ fontSize: 14, color: "#64748b", marginBottom: 24 }}>Selecciona tu tipo de cuenta para continuar.</p>

            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 24 }}>
              {([
                { role: "executive", label: "Soy Asesor", desc: "Ejecutivo de crédito — requiere código de invitación", color: "#1e40af", bg: "#eff6ff", border: "#bfdbfe" },
        ...
        {step === "code" && (
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
              <button onClick={() => { setStep("role"); setSelectedRole(null); setCodeError(""); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#64748b", fontSize: 13, padding: 0 }}>← Regresar</button>
              {selectedRole && (
            ...
            <h2 style={{ fontSize: 19, fontWeight: 700, color: "var(--navy-800)", marginBottom: 6 }}>Código de acceso</h2>
            ...
            <label style={{ fontSize: 12, fontWeight: 600, color: "#374151", textTransform: "uppercase", letterSpacing: "0.05em" }}>
              {selectedRole === "admin" ? "Código de administrador" : "Código de invitación"}
            </label>
            <input
              value={code}
              onChange={e => setCode(e.target.value.toUpperCase())}
              placeholder={selectedRole === "admin" ? "Código de 8 caracteres" : "Ej: A1B2C3D4"}
              maxLength={16}
              autoFocus
              style={{ ...inputStyle, marginTop: 8, marginBottom: codeError ? 6 : 20, fontFamily: "monospace", fontSize: 18, fontWeight: 700, letterSpacing: "0.15em", textAlign: "center" }}
            ...
            <button
              ...
              style={{ width: "100%", padding: "13px", background: "var(--accent)", color: "white", border: "none", borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: "pointer", opacity: codeLoading || !code.trim() ? 0.6 : 1 }}
            >
        ...
        {step === "form" && (
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
              <button onClick={() => { setStep("code"); setFormError(""); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#64748b", fontSize: 13, padding: 0 }}>← Regresar</button>
              {codeRole && (
                <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20, background: codeRole === "admin" ? "#ede9fe" : codeRole === "executive" ? "#dbeafe" : "#dcfce7", color: codeRole === "admin" ? "#7c3aed" : codeRole === "executive" ? "#1e40af" : "#166534" }}>
                  {ROLE_LABEL[codeRole]}
                </span>
              )}
            </div>

            <h2 style={{ fontSize: 19, fontWeight: 700, color: "var(--navy-800)", marginBottom: 20 }}>Crea tu cuenta</h2>
        ...
        {step === "done" && (
          <div style={{ textAlign: "center", padding: "24px 0" }}>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "var(--navy-800)", marginBottom: 8 }}>Cuenta creada</h2>
